...
BEGIN
    -- Add policies safely
    DROP POLICY IF EXISTS "Batches are viewable by authenticated users" ON batches;
...
BEGIN
    -- Add policies safely
    DROP POLICY IF EXISTS "Bags are viewable by authenticated users" ON bags;
    DROP POLICY IF EXISTS "Bags are insertable by authenticated users" ON bags;
...
BEGIN
    -- Add policies safely
    DROP POLICY IF EXISTS "Scans are viewable by authenticated users" ON scans;
...
BEGIN
    -- Add policies safely
    DROP POLICY IF EXISTS "Illegal dumping reports are viewable by authenticated users" ON illegal_dumping;
...
BEGIN
    -- Add policies safely
    DROP POLICY IF EXISTS "Illegal dumping history is viewable by authenticated users" ON illegal_dumping_history;
